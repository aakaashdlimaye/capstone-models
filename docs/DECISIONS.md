...
## 12. Second review round

A second review of the fixed pipeline found six further problems. All six are
addressed below.

| # | Decision | Rationale |
|---|---|---|
| 12.1 | The README's "What the run found" section is **generated from the CSVs** by `src/report.py`, between `<!-- FINDINGS:START -->` and `<!-- FINDINGS:END -->` markers | It had been written by hand and, after the first round of fixes, still quoted a decomposition that no longer existed (54% "static formulation", window-level intervals) while `RESULTS.md` quoted the corrected one. Two hand-maintained summaries of the same numbers will always drift; one generated and one hand-written cannot. |
| 12.2 | The distribution-shift flag now requires the recovery to clear **both** a firm-clustered interval and a floor of 10% of the drop (`MIN_RECOVERY_SHARE`), and the table carries an explicit `shift_verdict` sentence | The old flag compared two drops by sign alone, with no interval. At h=4 it read `True` on a change from -0.03765 to -0.03760 -- five parts in a hundred thousand. The honest conclusion at every horizon is that refitting two years closer to the test window recovers nothing, so distribution shift does not explain the A->B ROC-AUC drop. |
| 12.3 | The XGBoost t-0 vs 8-quarter comparison on Altman's five ratios is now **five seeds per rung with a firm-clustered interval on the difference**, in `decomposition_tree_ladder_tests.csv` | It is the cleanest evidence in the study -- the time-axis question with no neural network in it -- and it was a single deterministic fit quoted without an interval. A headline result cannot rest on a point estimate. |
| 12.4 | H1 gains **H1b**, which drops the four year-on-year growth ratios (r21 revenue, r22 net income, r23 assets, r24 equity) and repeats the comparison | "window_helps = False" is an observation, not an explanation. The candidate explanation is that a single t-0 row already carries four quarters of history through the growth ratios, so the window is re-supplying what the model has. The test reports what the growth ratios are worth to each model separately and whether the window-minus-t0 gap widens once they are gone; both are needed, because the gap could widen for unrelated reasons. |
| 12.5 | Scores that are not probabilities are **Platt-scaled on validation** before calibration, and the table names the scaling in a `scaling` column | An ECE of 0.49 on a raw Altman Z says only that a Z-score is not a probability, which nobody needed a number for. Platt scaling is a one-variable logistic fitted on validation and applied once to test: monotone, so ROC-AUC and PR-AUC are provably unchanged (unit-tested), and it turns the question into the one worth asking -- can this score's ranking be turned into calibrated probabilities? Fitted on val only, so no leakage. |
| 12.6 | Every tabular learner now runs over the **same five seeds** as the neural models; `{model}_{h}` is the seed-averaged ensemble and `{model}_s{seed}_{h}` each draw | The significance tests were comparing a single XGBoost fit against five-seed neural ensembles. XGBoost, the random forest and the SGD-based SVM are all stochastic, so a single fit is one draw. The bias ran against the tabular model, which made the "tied" results conservative rather than wrong -- but conservative in an unstated direction is still a defect. Logistic regression is deterministic and honestly reports a standard deviation of zero. |
